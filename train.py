...
def reconstruction(args):
    ...
    eikonal_reg_weight = args.eikonal_weight
    print(f"initial eikonal_reg_weight: {eikonal_reg_weight}")
    ...
    for iteration in pbar:
        ...
        rgb_map, alphas_map, depth_map, weights, uncertainty, sd, points_sampled = renderer(
            rays_train,
            tensorf,
        ...
        if eikonal_reg_weight > 0:
            eikonal_reg_weight *= lr_factor
            loss_eikonal = tensorf.eikonal_loss(sd, points_sampled) * eikonal_reg_weight
            total_loss = total_loss + loss_eikonal
            summary_writer.add_scalar(
                "train/reg_eikonal", loss_eikonal.detach().item(), global_step=iteration
            )
